...
int main(int argc, char **argv) {
    ...
    MPI_Status status;
    int my_id, root_process, i, num_rows, num_procs, an_id, num_rows_to_receive,
    ...
    if (my_id == root_process) {
        ...
        for (an_id = 1; an_id < num_procs; an_id++) {
            ...
            MPI_Send(&array[start_row], num_rows_to_send, MPI_INT, an_id, send_data_tag,
                     MPI_COMM_WORLD);
        }
        ...
        // Finally collect the partial sums from the slave processes, print them
        // and add them to the grand sum.
        for (an_id = 1; an_id < num_procs; an_id++) {
            MPI_Recv(&partial_sum, 1, MPI_LONG, MPI_ANY_SOURCE, return_data_tag,
        ...
        // Children processes
        MPI_Recv(&num_rows_to_receive, 1, MPI_INT, root_process, send_data_tag,
                 MPI_COMM_WORLD, &status);
        MPI_Recv(&array2, num_rows_to_receive, MPI_INT, root_process, send_data_tag,
                 MPI_COMM_WORLD, &status);
